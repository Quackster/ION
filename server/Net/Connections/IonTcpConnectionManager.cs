...
namespace Ion.Net.Connections
{
    ...
    public class IonTcpConnectionManager
    {
        ...
        /// </summary>
        /// <param name="pConnection">The IonTcpConnection instance representing the new connection to handle.</param>
        public void HandleNewConnection(IonTcpConnection pConnection)
        {
            ...
            mConnections.Add(pConnection.ID, pConnection);

            // Create session for new client
            IonEnvironment.GetHabboHotel().GetClients().StartClient(pConnection.ID);
        }
        public void DropConnection(uint clientID)
        {
            ...
            if (pConnection != null)
            {
                IonEnvironment.Log.WriteInformation("Dropped OkapiTcpConnection [" + clientID + "] of " + pConnection.ipAddress);
