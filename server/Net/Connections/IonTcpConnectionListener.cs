    ...
    public class IonTcpConnectionListener
    {
        ...
        public IonTcpConnectionListener(string sLocalIP, int Port, IonTcpConnectionManager pManager)
        {
            ...
            if(!IPAddress.TryParse(sLocalIP, out pIP))
            {
                ...
                IonEnvironment.Log.WriteWarning(string.Format("Connection listener was unable to parse the given local IP address '{0}', now binding listener to '{1}'.", sLocalIP, pIP.ToString())); 
            }
            ...
            IonEnvironment.Log.WriteLine(string.Format("IonTcpConnectionListener initialized and bound to {0}:{1}.", pIP.ToString(), Port.ToString()));
        }
        #endregion
