    ...
    public class IonTcpConnection
    {
        ...
        public void SendData(byte[] Data)
        {
            if (this.Alive)
            {
                ...
                {
                    IonEnvironment.Log.WriteUnhandledExceptionError("IonTcpConnection.Send", ex);
                }
            }
        }
        ...
        {
            IonEnvironment.Log.WriteLine(" [" + mID + "] <-- " + pMessage.Header + pMessage.GetContentString());
        ...
        private void WaitForData()
        {
            if (this.Alive)
            {
                ...
                {
                    IonEnvironment.Log.WriteUnhandledExceptionError("IonTcpConnection.WaitForData", ex);
                    ConnectionDead();
                }
            }
        }
        private void DataReceived(IAsyncResult iAr)
        {
            ...
            {
                IonEnvironment.Log.WriteUnhandledExceptionError("IonTcpConnection.DataReceived", ex);
