    ...
    public class DatabaseManager
    {
        ...
        private void MonitorClientsLoop()
        {
            while (true)
            {
                        ...
                        for (int i = 0; i < mClients.Length; i++)
                        {
                            if (mClients[i].State != ConnectionState.Closed)
                            {
                                if (mClients[i].Inactivity >= 60) // Not used in the last %x% seconds
                                {
                                    ...
                                    IonEnvironment.Log.WriteInformation("Disconnected database client #" + mClients[i].Handle);
                                }
                            }
                        }
                    }
                ...
                {
                    IonEnvironment.Log.WriteError(ex.Message);
                }
            }
        }
        /// <summary>
        ...
        public DatabaseClient GetClient()
        {
            ...
            lock (this)
            {
                ...
                for (uint i = 0; i < mClients.Length; i++)
                {
                    ...
                    if (mClientAvailable[i] == true)
                    {
                        ...
                        if (mClients[i].State == ConnectionState.Closed)
                        {
                            ...
                            IonEnvironment.Log.WriteInformation("Opening connection for database client #" + mClients[i].Handle);
                        }

                        // Is this client ready?
                        if (mClients[i].State == ConnectionState.Open)
                        {
                            IonEnvironment.Log.WriteLine("Handed out client #" + mClients[i].Handle);
                ...
                IonEnvironment.Log.WriteLine("Handed out anonymous client.");
        ...
        public void ReleaseClient(uint Handle)
        {
            if (mClients.Length >= (Handle - 1)) // Ensure client exists
            {
                ...
                IonEnvironment.Log.WriteLine("Released client #" + Handle);
            }
        }
