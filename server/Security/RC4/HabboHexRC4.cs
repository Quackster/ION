...
namespace Ion.Security.RC4
{
    public class HabboHexRC4
    {
        ...
        public void Initialize(int tMyKey)
        {
            ...
            var ckey = new Dictionary<int, byte>();
            var fakeKey = new Dictionary<int, byte>();
            ...
            {
                fakeKey[i] = (byte)i;
                var keySkip = (prevKey % 19) - (i % 7);
                m *= -1;
                var nkey = ARTIFICIAL_KEY[Math.Abs(tOffset + (i * m * keySkip) + keySkip) % ARTIFICIAL_KEY.Length];
                prevKey = nkey;
                ckey[i] = nkey;
                i++;
            }
        ...
        public byte MixTable()
        {
            ...
            this.j = (byte)((j + pSBox[i]) % 256);

            // Swap table fields
            byte bSwap = pSBox[this.i];
